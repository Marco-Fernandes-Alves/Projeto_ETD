...
class SentimentModel(BaseModel):
    ...
    nike_sentiment: float = Field(ge=-10, le=10)
    lvmh_sentiment: float = Field(ge=-10, le=10)
    inditex_sentiment: float = Field(ge=-10, le=10)
    general_sentiment: float = Field(ge=-10, le=10)
            ...
            for idx, row in df.iterrows():
                try:
                    SentimentModel(
                        ...
                        nike_sentiment=row['nike_sentiment'],
                        lvmh_sentiment=row['lvmh_sentiment'],
                        inditex_sentiment=row['inditex_sentiment'],
                        general_sentiment=row['general_sentiment']
                    )
                except ValidationError as e:
                    errors += 1
                    if errors <= 3:
            ...
            for idx, row in df.iterrows():
                try:
                    GoldModel(
                        ...
                        sentiment_score=row['sentiment_score'],
                        nike_sentiment=row['nike_sentiment'] if not pd.isna(row['nike_sentiment']) else 0.0,
                        lvmh_sentiment=row['lvmh_sentiment'] if not pd.isna(row['lvmh_sentiment']) else 0.0,
                        inditex_sentiment=row['inditex_sentiment'] if not pd.isna(row['inditex_sentiment']) else 0.0,
                        general_sentiment=row['general_sentiment'] if not pd.isna(row['general_sentiment']) else 0.0
                    )
                except ValidationError as e:
                    errors += 1
                    if errors <= 3:
