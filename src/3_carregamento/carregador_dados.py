...
class ModaDataLoader:
    def __init__(self, db_path="src/3_carregamento/dados/moda_analytics.db", gold_csv_path="src/2_transformacao/data/processados/ouro_analise_moda.csv"):
        self.db_path = db_path
        self.gold_csv_path = gold_csv_path
        
        # Garantir que a diretoria da base de dados existe
    ...
    def criar_esquema(self, conn):
        ...
        # 4. Tabela de Factos: Mercado de Moda
        cursor.execute("""
        CREATE TABLE IF NOT EXISTS facto_mercado (
            date TEXT PRIMARY KEY,
            nike_close REAL NOT NULL,
            nike_volume REAL NOT NULL,
            lvmh_close REAL NOT NULL,
            lvmh_volume REAL NOT NULL,
            inditex_close REAL NOT NULL,
            inditex_volume REAL NOT NULL,
            FOREIGN KEY (date) REFERENCES dim_tempo (date),
        ...
        # 5. Criar View Consolida para a Camada de Visualização
        cursor.execute("""
        ...
        conn.commit()
        logging.info("Tabelas, índices e view analítica criados com sucesso!")
    ...
    def carregar_dados(self):
        ...
        # Assegurar tipo datetime para processamento de datas
        ...
        try:
            # 1. Preparar e carregar dim_tempo
            ...
            # 2. Preparar e carregar dim_macroeconomia
            ...
            # 3. Preparar e carregar dim_sentimento
            logging.info("A povoar dim_sentimento...")
            sent_df = df[['date', 'sentiment_score']].drop_duplicates()
            sent_df.to_sql('dim_sentimento', conn, if_exists='append', index=False)

            # 4. Preparar e carregar facto_mercado
    ...
    def realizar_testes_qualidade(self):
        ...
        # Testar se a View consolidada tem o mesmo número de linhas que a facto_mercado
        ...
        # Gravar relatório de validação na pasta do projeto
        report_path = "src/3_carregamento/relatorio_valida_carga.md"
